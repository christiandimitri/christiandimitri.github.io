...
DXF is a forty-year-old text format that construction still runs on. Floor-plan files in the wild reach hundreds of megabytes: nested block references, hatches with thousands of loops, polylines by the million. For a construction-robotics platform I work on, those files have to open in a browser, render at 60 fps, and stay editable. These notes are format-agnostic enough to be useful beyond DXF, which is good, because I can't show you the product.

## Parse where the memory is: C++ → WASM

JavaScript DXF parsers exist and are fine, until the file is 300 MB of text producing millions of entities, at which point GC pressure and per-entity object overhead eat the tab. Our core moved to **C++ compiled to WebAssembly** with [embind](https://emscripten.org/docs/porting/connecting_cpp_and_javascript/embind.html) bindings. The parser produces packed typed arrays (positions, entity IDs, style indices) that cross the WASM boundary once, as views, not object graphs.
...
Hover and selection in CAD means *this* entity changes color, now. Rebuilding vertex colors for a million-vertex batch on every mouse move is not a plan, and yes, I tried it first. The pipeline that works:

- Every vertex carries its **entity ID** as an attribute.
- A small **data texture** maps entity ID to state: hovered, selected, hidden, custom color.
...
None of these techniques is novel on its own. Game engines have done all three for decades. What's new is the sum crossing into AEC tooling through a browser tab: the heaviest desktop-only format in construction, made portable. The gap between "CAD software" and "web app" is now an engineering decision, not a platform limitation.
